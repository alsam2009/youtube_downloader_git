...
"""
Программа используется для докачки информации с канала в ручном режиме
после возникновения ошибки в процессе отработки основного скрипта.
...
def get_channel_data(channel_url):
    ...
    try:
        ...
        make_channel_dir(channel)
        len_channel = len(c.videos)
        progress_view = len_channel // 10
        counter = progress_view

        try:
            # Значения начала диапазона выбирается по последней строке после ошибки, конец - Len(c.videos)
            for i in range(682, 715):
                video = c.videos[i]
                title = video.title
                url = c.video_urls[i]
                ...
                if progress_view > 1:
                    progress_view = make_progress_view(
                        i, progress_view, len_channel, counter)
...
def channel_parser():
    ...
    start = datetime.now()
    count_error = get_channel_data(channel_url)
    end = datetime.now()
    print(
